...
from ilp_solver import TSPSolver

gtsp_adj_mat = np.array([\
...
set_map = np.array([1, 2, 2, 2, 0, 2, 2, 1, 1, 1, 1, 2, 2, 2])

# for i in range(len(set_map)):
#     for j in range(len(set_map)):
#         if(set_map[i] == set_map[j]):
#             gtsp_adj_mat[i, j] = -1

tsp_mat = gtsp_to_tsp(gtsp_adj_mat, set_map)
solver = TSPSolver()

tour = solver.solve_exact_tsp(tsp_mat)
set_order = [set_map[i] for i in tour]
